...
describe Postcodesio do
  ...
  before(:all) do
    # @postcodes_array = ['SE3 8PF', 'SE9 6RJ', 'B601JA', 'SW155DU', 'B601JA', 'SW155DU']
  ...
  it "should return a country which is one of the four constituent countries of the UK" do
    @postcodesio.get_all_country_results.each do |country|
      expect(country).to eq("England") | eq("Wales") | eq("Scotland")| eq("Northern Ireland") | be(nil)
    end
  end
  ...
  it "should return a european_electoral_region string" do
    @postcodesio.get_all_european_electoral_region_results.each do |eur|
      expect(eur).to be_kind_of(String) | be(nil)
    end
  end
  ...
  it "should return a region string" do
    @postcodesio.get_all_region_results.each do |region|
      expect(region).to be_kind_of(String) | be(nil)
    end
  end

  it "should return a parish string" do
    @postcodesio.get_all_parish_results.each do |par|
      expect(par).to be_kind_of(String) | be(nil)
    end
  end
  ...
  it "should return a msoa string" do
    @postcodesio.get_all_msoa_results.each do |msoa|
      expect(msoa).to be_kind_of(String) | be(nil)
    end
  end
